...
function criarCaminhoOnduladoSelo(cx, cy, r, numScallops, depth) {
        ...
        for (let i = 0; i < numScallops; i++) {
          ...
          const a2    = (2 * i + 1) * step;
          const rVal  = r - depth;
          const x1 = cx + rVal * Math.cos(a1),  y1 = cy + rVal * Math.sin(a1);
          const xP = cx + r   * Math.cos(aPeak), yP = cy + r   * Math.sin(aPeak);
          const x2 = cx + rVal * Math.cos(a2),  y2 = cy + rVal * Math.sin(a2);
          if (i === 0) path.push(`M ${x1.toFixed(2)} ${y1.toFixed(2)}`);
          path.push(`Q ${xP.toFixed(2)} ${yP.toFixed(2)} ${x2.toFixed(2)} ${y2.toFixed(2)}`);
        }
        path.push('Z');
        return path.join(' ');
      }
      ...
      function gerarQRCodeValidador(hash) {
        ...
        try {
          new QRCode(container, {
            ...
            colorDark: "#394353", colorLight: "#ffffff",
            correctLevel: QRCode.CorrectLevel.H
          });
        } catch (e) { console.error("QRCode falhou:", e); }
      }
