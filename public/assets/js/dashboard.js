;(function () {
  ...
  function stats(modules) {
    ...
    var notes = modules
      .filter(function (module) {
        return module.nota !== null
      })
      .map(function (module) {
        return module.nota
      })
    var best = notes.length ? Math.max.apply(null, notes) : 0
    var lastModule = modules
      .slice()
      .reverse()
      .find(function (module) {
        return module.ultimaNota > 0
      })
    var last = lastModule ? lastModule.ultimaNota : 0
    var average = notes.length
      ? Math.round(
          notes.reduce(function (sum, note) {
            return sum + note
          }, 0) / notes.length
        )
      : 0
  ...
  function isModuleProgressComplete(module, progress) {
    return (
      progress >= PASSING_AVERAGE ||
      module.tentativasUsadas >= module.tentativas
    )
  }
  ...
  function moduleProgressTemplate(module) {
    ...
    return (
      '<div class="module-progress-row"><strong>' +
      module.nome +
      '</strong><div class="module-progress-details"><div class="progress"><div class="progress-bar ' +
      barClass +
      '" data-width="' +
      progress +
      '"></div></div><span class="module-progress-percent">' +
      progressLabel +
      '</span></div></div>'
    )
  }
  ...
  function moduleStatusTemplate(module) {
    return (
      '<div class="status-item"><span class="badge ' +
      ...
      module.tentativasUsadas +
      '/' +
      module.tentativas +
      ' tentativas usadas</p></div></div>'
    )
  }
  ...
  function renderCertificateCta(modules, data) {
    ...
    $(selectors.certificateCta).innerHTML = canIssue
      ? '<h2>Parabens! Voce esta aprovado.</h2><p class="lead">Voce completou todos os modulos com media de <strong class="text-success">' +
        data.average +
        '%</strong>.</p><button class="button button-secondary" id="btn-emitir">Emitir Meu Certificado</button>'
      : '<h2>Continue sua jornada</h2><p class="lead">Faltam ' +
        (modules.length - data.complete) +
        ' modulo(s) para concluir a certificacao.</p><a class="button button-primary" href="modulos.html">Continuar Estudando</a>'

    if (canIssue) {
      document
        .getElementById('btn-emitir')
        .addEventListener('click', async function () {
          try {
            const response = await apiFetch('/api/certificados/emitir', {
              method: 'POST'
            })

            if (response.ok) {
              const result = await response.json()
              window.location.href = '/certificado/' + result.certificadoHash
            } else if (response.status === 403) {
              const error = await response.json()
              alert(
                'Bloqueio de Emissão: ' +
                  (error.message || 'Requisitos não preenchidos.')
              )
            } else {
              alert('Erro ao processar emissão. Tente novamente mais tarde.')
            }
          } catch (err) {
