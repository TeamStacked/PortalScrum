...
router.post("/cadastro", async function (req, res) {
  ...
  try {
    ...
    /* seria aqui que ficaria a criptografia (senha.trim) */

    const response = await createUsuarios(cpf, nome, email, senha);
    if (response && !response.error) {
      ...
      return res.status(404).json({erro: response.error || 'Erro ao cadastrar.' });
    }
  } catch (err) {
